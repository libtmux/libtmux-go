...
type entrySpec struct {
	...
	// Unit is what tmux's own options table calls this option's number:
	// "milliseconds" or "seconds". An option that has one is a duration.
...
func generateOptions(spec optionSpec) ([]byte, error) {
	...
	// A duration option needs time; nothing else this file emits does.
	if slices.ContainsFunc(spec.Options, func(entry entrySpec) bool { return entry.Unit != "" }) {
...
func valueKindName(entry entrySpec) string {
	...
	case "int64", "time.Duration":
		// tmux stores a duration as a number of its own unit, so what
		// crosses the wire is an integer either way.
		return "Int64"
	case "string":
		return "String"
...
func writeScalarSetter(output *bytes.Buffer, entry entrySpec, receiver string) {
	...
	case "time.Duration":
		// tmux stores this one as a whole number of its unit, so a value it
		// cannot hold is refused here rather than silently truncated.
		fmt.Fprintf(
			output,
			"encoded, err := encodeTypedOptionDuration(%s, value, %s)\nif err != nil { return err }\n",
			strconv.Quote(entry.Name),
			goUnit(entry.Unit),
		)
		encoded = "encoded"
...
func decodedValueExpression(entry entrySpec) string {
	...
	if entry.Unit != "" {
		return "time.Duration(" + field + ") * " + goUnit(entry.Unit)
	}
	return field
}

// goUnit names the [time] constant for one of tmux's option units.
func goUnit(unit string) string {
	switch unit {
	case "milliseconds":
	...
	case "seconds":
		return "time.Second"
	default:
		panic("validated option spec has unknown unit " + unit)
	}
}
