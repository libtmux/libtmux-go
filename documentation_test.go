...
var exampleWorkflowSources = []string{
	...
	"_examples/snapshot-browser/main.go",
	"_examples/option-hook-editing/main.go",
	"_examples/filter-query/main.go",
	"_examples/environment/main.go",
...
func TestExportedAPIDeclarationsHaveDocumentation(t *testing.T) {
	...
	packages := []string{moduleRoot, filepath.Join(moduleRoot, "tmuxq"), filepath.Join(moduleRoot, "tmuxtest")}
...
func TestREADMEGoFencesCompile(t *testing.T) {
	t.Parallel()

	moduleRoot := documentationModuleRoot(t)
	readme, err := os.ReadFile(filepath.Join(filepath.Dir(moduleRoot), "README.md"))
	if err != nil {
		t.Fatal(err)
	}
	programs := markdownFences(string(readme), "go")
	if len(programs) == 0 {
		t.Fatal("README.md contains no Go program")
	}
	for index, program := range programs {
		workspace := t.TempDir()
		if err := os.WriteFile(filepath.Join(workspace, "main.go"), []byte(program), 0o600); err != nil {
			t.Fatal(err)
		}
		goMod := "module readme-example\n\ngo 1.23.0\n\n" +
			"require github.com/tmux-python/libtmux/golang v0.0.0\n\n" +
			"replace github.com/tmux-python/libtmux/golang => " + moduleRoot + "\n"
		if err := os.WriteFile(filepath.Join(workspace, "go.mod"), []byte(goMod), 0o600); err != nil {
			t.Fatal(err)
		}
		build := exec.Command("go", "build", "-o", filepath.Join(workspace, "readme-example"), ".")
		build.Dir = workspace
		build.Env = append(os.Environ(), "GOWORK=off")
		if output, err := build.CombinedOutput(); err != nil {
			t.Fatalf("build README Go fence %d: %v\n%s", index+1, err, output)
		}
	}
}

func TestDocumentationIndexAdvertisesGoSupportWindow(t *testing.T) {
	t.Parallel()

	moduleRoot := documentationModuleRoot(t)
	index, err := os.ReadFile(filepath.Join(filepath.Dir(moduleRoot), "docs", "index.md"))
	if err != nil {
		t.Fatal(err)
	}
	const supportWindow = "Unreleased · Go 1.23 · tmux 3.2a–3.7b"
	if !strings.Contains(string(index), supportWindow) {
		t.Fatalf("docs index does not contain Go support window %q", supportWindow)
	}
}

func markdownFences(markdown, language string) []string {
	opening := "```" + language
	lines := strings.Split(markdown, "\n")
	programs := make([]string, 0, 1)
	for index := 0; index < len(lines); index++ {
		...
		for index++; index < len(lines); index++ {
			...
			programs = append(programs, strings.Join(lines[start:index], "\n")+"\n")
			break
		}
	}
	return programs
...
func TestExampleWorkflowsBuildAndRun(t *testing.T) {
	...
	workflows := []struct {
		...
		{name: "quickstart", packagePath: "./_examples/quickstart"},
		{name: "snapshot-browser", packagePath: "./_examples/snapshot-browser"},
		{name: "option-hook-editing", packagePath: "./_examples/option-hook-editing"},
		{name: "filter-query", packagePath: "./_examples/filter-query"},
		{name: "environment", packagePath: "./_examples/environment"},
		{name: "control-mode-subscribe", packagePath: "./_examples/control-mode-subscribe", testBinary: true},
	}
...
func taskIndexSymbols(t *testing.T, moduleRoot string) []string {
	...
	fileSet := token.NewFileSet()
	packages, err := parser.ParseDir(fileSet, moduleRoot, nil, parser.ParseComments)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTaskIndexEntryPointsCarryRunnableExamples(t *testing.T) {
	...
	moduleRoot := documentationModuleRoot(t)
	symbols := taskIndexSymbols(t, moduleRoot)
	if len(symbols) == 0 {
