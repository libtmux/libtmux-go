...
- Remove `Color88`. tmux 3.2a and later reject `-8`, so `ServerOptions.Colors`
  ...
  (#15)
- `Running.Kill` now sends its SIGKILL through a backgrounded `run-shell` and
  returns once tmux has queued the request, not once the kill ran; a
  foreground `run-shell` held the caller until tmux forked, ran and reaped
  that job, turning an ordinarily instant `Kill` into an occasional
  multi-second one under load. Call `Running.Wait` to observe whether the
  process actually died. (#15)
